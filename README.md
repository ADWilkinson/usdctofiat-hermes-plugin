...
- `platform`: payment rail such as `revolut`, `venmo`, or `monzo`. Combined with `currency`, the pair must be one EscrowV2 actually settles.
...
`platform` and `currency` together are the check EscrowV2 actually makes. The client validates each argument on its own, so `venmo`/`EUR`, `monzo`/`USD` or `zelle`/`MXN` — pairs a model writes when the user says "cash out euros on Venmo" — each encode a well-formed `createDeposit` that reverts `CurrencyNotSupported` after the approve is signed. The plugin refuses a pair the verifier is not registered for, before `prepare` opens. `venmo`, `cashapp`, `chime` and `zelle` settle USD; `monzo` is GBP.
